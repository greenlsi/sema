...
domotic_control (void* ignore)
{
    ...
    int valid_code = 0;
    code_fsm = fsm_new_code (&valid_code, &gpio_activated[GPIO_BUTTON_CODE]);
    alarm_fsm = fsm_new_alarm (&valid_code, GPIO_PIR, GPIO_ALARM);
    light_fsm = fsm_new_light (&gpio_activated[GPIO_BUTTON_LIGHT], GPIO_ALARM);

    EventHandler eh_code, eh_alarm, eh_light;
    reactor_init ();

    event_handler_init (&eh_code,  3, code_func);
    event_handler_init (&eh_alarm, 2, alarm_func);
    event_handler_init (&eh_light, 1, light_func);
